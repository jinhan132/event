...
header("Content-Type:text/html;charset=utf-8");
	...
	$sql = "select date_format(user_basic.reg_time, '%Y-%m-%d') as reg_time, user_basic.n_code, count(*) from user_basic, quiz_user where user_basic.uid=quiz_user.uid and quiz_user.grade > 0 and user_basic.reg_time like $size_date  group by date_format(user_basic.reg_time, '%Y-%m-%d'), user_basic.n_code";

	$query = mysql_query($sql, $con);
	while($data = mysql_fetch_array($query)){
		$reg_time = $data[0];
		$n_code = $data[1];
		$reg_code[$reg_time][$n_code] = $data[2];
	}
...
<script language='JavaScript'>
	<!--
		...
		function sendCal(str) {
			...
			document.calForm.action = "idct_new.html";
			document.calForm.submit();
		}
	//-->
	</script>
...
$brcnt = 0;
for($k=0; $k<=3; $k++){
	if($reg_code[$t_day][$k] > 0){
		if($k==0){
			echo "<img src='//img.tygem.com/2019/img/rank/rank_kr.jpg'> ".$reg_code[$t_day][$k]."<br>";
			$brcnt++;
		}
		if($k==1){
			echo "<img src='//img.tygem.com/2019/img/rank/rank_jp.jpg'> ".$reg_code[$t_day][$k]."<br>";
			$brcnt++;
		}
		if($k==2){
			echo "<img src='//img.tygem.com/2019/img/rank/rank_cn.jpg'> ".$reg_code[$t_day][$k]."<br>";
			$brcnt++;
		}
		if($k==3){
			echo "<img src='//img.tygem.com/2019/img/common/ft_usa.jpg'> ".$reg_code[$t_day][$k]."<br>";
			$brcnt++;
		}
		if($k==4){
			echo "<img src='//img.tygem.com/2019/img/common/ft_tai.jpg'> ".$reg_code[$t_day][$k]."<br>";
			$brcnt++;
		}
	}
	$day_total += $reg_code[$t_day][$k];
}
for($b=$brcnt;$b<3;$b++){
	echo "<br>";
}
