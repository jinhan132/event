	var target;
	var stime;
	document.write("<div id=minical oncontextmenu='return false' ondragstart='return false' onselectstart='return false' style=\"width:360px; background:buttonface; margin:0; padding:0;margin-top:0;width:360;display:none;position:absolute; z-index: 9999;border: 1px solid black;\"></div>");
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML = "<TABLE  WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:10pt;font-family:Tahoma;' bgcolor='#EDF0F5'>"
			+ "<TR ALIGN=CENTER><TD COLSPAN=7 nowrap=nowrap ALIGN=CENTER><SPAN TITLE='' STYLE=cursor:pointer; onClick='Show_cal("+intPrevYear+","+intPrevMonth+",1);'> << </SPAN> "
			+ "<B STYLE=color:red>"+get_Yearinfo(intThisYear,intThisMonth,intThisDay)+get_Monthinfo(intThisYear,intThisMonth,intThisDay)+"</B>"
			+ " <SPAN TITLE='' STYLE=cursor:pointer; onClick='Show_cal("+intNextYear+","+intNextMonth+",1);'> >> </SPAN></TD></TR>"
			+ "</TABLE><table  border=\"1\" cellspacing=\"0\" cellpadding=\"0\" bordercolordark=\"#ffffff\" bordercolorlight=\"#C4CEDC\" width=\"360\" align=\"center\"><TR ALIGN=CENTER  height=\"24\" bgcolor=\"#C4CEDC\"><TD><font color=\"#08246B\"><b>Su</b></font></TD><TD><font color=\"#08246B\"><b>Mo</b></font></TD><TD><font color=\"#08246B\"><b>Tu</b></font></TD><TD><font color=\"#08246B\"><b>We</b></font></TD><TD><font color=\"#08246B\"><b>Th</b></font></TD><TD><font color=\"#08246B\"><b>Fr</b></font></TD><TD><font color=\"#08246B\"><b>Sa</b></font></TD></TR>";
	...
	for (intLoopWeek=1; intLoopWeek < 7; intLoopWeek++) {						
		...
		for (intLoopDay=1; intLoopDay <= 7; intLoopDay++) {						
			if (intThirdWeekday > 0) {
				...
				if(intThisMonth == 1){
					Cal_HTML += "<TD height=\"30\" onClick=doClick(); title="+intPrevYear+"-"+day2(intPrevMonth).toString()+"-"+day2(pevMonthDay).toString()+" STYLE=\"cursor:pointer;\" style=\"background-color:#F7F7FF;\">"+pevMonthDay;
				}else{
					Cal_HTML += "<TD height=\"30\" onClick=doClick(); title="+intThisYear+"-"+day2(intPrevMonth).toString()+"-"+day2(pevMonthDay).toString()+" STYLE=\"cursor:pointer;\" style=\"background-color:#F7F7FF;\">"+pevMonthDay;
				}
				intPrevDay++;
				intThirdWeekday--;
			} else {
				if (thirdPrintDay > intLastDay) {								
					...
					if(intThisMonth == 12){
						Cal_HTML += "<TD height=\"30\" onClick=doClick(); title="+intNextYear+"-"+day2(intNextMonth).toString()+"-"+day2(NextPrintDay).toString()+" STYLE=\"cursor:pointer;\" bgcolor=\"#F7F7FF\">"+(thirdPrintDay-intLastDay);
					}else{
						Cal_HTML += "<TD height=\"30\" onClick=doClick(); title="+intThisYear+"-"+day2(intNextMonth).toString()+"-"+day2(NextPrintDay).toString()+" STYLE=\"cursor:pointer;\" bgcolor=\"#F7F7FF\">"+(thirdPrintDay-intLastDay);
					}
				} else {														
					Cal_HTML += "<TD height=\"30\" onClick=doClick(); title="+intThisYear+"-"+day2(intThisMonth).toString()+"-"+day2(thirdPrintDay).toString()+" STYLE=\"cursor:pointer;\"";
	...
	Cal_HTML += "<tr><td colspan=\"7\" align=\"center\" bgcolor=\"#DAE1EB\" height=\"30\">Today(<a href=\"#\" onClick=doClick(); title=\""+DNowThisYear+"-"+day2(DNowThisMonth).toString()+"-"+day2(DNowThisDay).toString()+"\">"+DNowThisYear+"-"+day2(DNowThisMonth).toString()+"-"+day2(DNowThisDay).toString()+"</a>) | <a href=\"#\" style=\"color: black; text-decoration: none\" onClick=doClick();>Close</a></td></tr>"
	Cal_HTML += "</TABLE>";
...
function get_Yearinfo(year,month,day) {											
	...
	str = "<SELECT onChange='Show_cal(this.value,"+month+","+day+");' ONMOUSEOVER=doOver(); style='color:#333333; height:24px;'>";
	for (i=min; i<=max; i++) {
		if (i == parseInt(year)) {
...
function get_Monthinfo(year,month,day) {										
	...
	str = "<SELECT onChange='Show_cal("+year+",this.value,"+day+");' ONMOUSEOVER=doOver(); style='color:#333333;height:24px;'>";
	for (i=1; i<=12; i++) {
		if (i == parseInt(month)) {
			str += "<OPTION VALUE="+i+" selected ONMOUSEOVER=doOver(); style='background-color:#ffffff'>"+i+"</OPTION>";
		} else {
